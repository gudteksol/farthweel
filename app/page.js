  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
        <span style={{ color: "#555" }}>[{tx.time}]</span>
        <span style={{ color: "#0f0", flex: 1 }}>BUY {tx.sol.toFixed(4)} SOL</span>
        <span style={{ color: TIER_COLORS[getFartTier(tx.sol)], whiteSpace: "nowrap" }}>
          {FART_FACES[getFartTier(tx.sol)]} {FART_LABELS[getFartTier(tx.sol)]}
        </span>
      </div>
      <div style={{ color: TIER_COLORS[getFartTier(tx.sol)], opacity: 0.6, marginTop: 2 }}>
        {"█".repeat(Math.min(Math.ceil(tx.sol * 2), 30))}
      </div>
      <div style={{ color: "#444", fontSize: 11, marginTop: 2 }}>sig: {tx.sig}</div>
    </div>
  );
...
export default function Fartwheel() {
  ...
  const fetchFromApi = useCallback(async () => {
    try {
      ...
      // On first load, just record what we know — don't fart for history
      ...
      // Find new transactions and trigger farts
      for (const tx of txs) {
        if (!knownSigs.current.has(tx.sig)) {
          ...
          // Clear "new" animation after a bit
          setTimeout(() => {
            setNewSigs((prev) => {
        ...
        <div style={{ maxHeight:500,overflowY:"auto",padding:"0 4px" }}>
          {transactions.length === 0 ? (
            ...
            transactions.map((tx, i) =>
              newSigs.has(tx.sig)
                ? <NewTxRow key={tx.sig + i} tx={tx} />
                : <TxRow key={tx.sig + i} tx={tx} />
            )
          )}
        </div>
